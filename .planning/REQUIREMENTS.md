...
- [ ] **DATA-02**: The status taxonomy is unified across all 9 pages — one StatusPill maps every Oban/Powertools state consistently.
- [ ] **DATA-03**: Tables/lists degrade gracefully at 320px (stacked/card fallback); all show explicit empty vs loading vs unavailable vs permission-denied states; long IDs/module names/URLs/stacktraces are handled (truncation + expansion/tooltip).
- [ ] **DATA-04**: Redaction/DisplayPolicy presentation is rendered through one shared component (no per-page reinvention).
