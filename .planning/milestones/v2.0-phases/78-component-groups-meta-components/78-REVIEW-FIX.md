...
Fresh phase verification subsequently found a separate framing defect: the canonical pixels for 10 activated overlay stories were clipped to their containing article. That was not a regression in the launcher fix, but it meant the full-matrix comparison above proved deterministic fragments. Gap closure now captures the exact confirmation/native-detail roots, asserts their heading and action/close control, refreshes exactly **120** overlay PNGs, and passes a new no-update Docker comparison **276/276**. This later evidence supersedes the overlay portion of the review-fix matrix while retaining the 80-path environment correction history.
