...
function fail(message) {
  throw new Error(`Invalid group baselines: ${message}`);
}
...
function readManifest() {
  ...
  if (manifest.schema_version !== 6) {
    fail(
      `schema_version must be 6, got ${JSON.stringify(manifest.schema_version)}`,
    );
  }

  for (const field of ["group_stories", "themes", "viewports"]) {
    if (!Array.isArray(manifest[field])) {
      fail(`${field} must be an array`);
    }
  }

  if (manifest.group_stories.length !== expectedStoryCount) {
    fail(
      `group_stories must contain ${expectedStoryCount} entries, got ${manifest.group_stories.length}`,
    );
  }
...
function expectedPaths(manifest) {
  ...
  for (const viewport of manifest.viewports) {
    ...
    for (const story of manifest.group_stories) {
      if (
        story.kind !== "group" ||
        !/^showcase\/group-[a-z0-9-]+$/.test(story.snapshot) ||
        !["none", "overlay"].includes(story.activation)
      ) {
        fail(
          `invalid group story ${JSON.stringify({
            id: story.id,
            kind: story.kind,
  ...
  if (
    ...
    fail(
      `manifest-derived matrix must contain 23 * 4 * 3 = 276 unique paths, got ${expected.length} paths and ${unique.size} unique paths`,
    );
  }
...
function actualGroupPaths() {
  return new Set(
    walk(screenshotRoot)
      .filter((file) => file.endsWith(".png"))
      .map(repoRelative)
      .filter((file) =>
        file.split("/").some((segment) => segment.startsWith("group-")),
      ),
  );
}
...
function reportSetDifference(expected, actual) {
  ...
  if (actual.size !== expectedCount) {
    fail(
      `actual group baseline cardinality must be ${expectedCount}, got ${actual.size}`,
    );
  }
}
...
if (args.some((arg) => arg !== "--changed-scope") || args.length > 1) {
  fail(
    "usage: node test/browser/support/verify-group-baselines.mjs [--changed-scope]",
  );
}
...
if (args[0] === "--changed-scope") {
  ...
  if (outside.length > 0) {
    fail(
      `changed screenshots outside group matrix (${outside.length}):\n${outside.join("\n")}`,
    );
  }

  console.log(
    `group baseline changed scope ok: ${changed.length} paths, all within ${expectedCount}`,
  );
} else {
  const actual = actualGroupPaths();
  reportSetDifference(expected, actual);
  console.log(`group baselines ok: ${actual.size}`);
}
