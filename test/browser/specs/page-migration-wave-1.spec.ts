...
type Phase79Recovery = 'expired' | 'drifted' | 'consumed' | 'skipped' | 'partial';
...
const schemaVersion = Number(
  (manifestSupport as unknown as { manifest?: { schema_version?: unknown } }).manifest
    ?.schema_version
);
const pageStories = (manifestSupport as FutureManifestSupport).pageStories;

if (schemaVersion !== 7 || !Array.isArray(pageStories) || pageStories.length !== 19) {
  throw new Error(
...
const expectedPageFamilyCounts = { overview: 3, cron: 8, limiters: 4, audit: 4 } as const;
const pageFamilyCounts = pageStories.reduce<Record<string, number>>((counts, story) => {
  counts[story.page] = (counts[story.page] ?? 0) + 1;
  return counts;
}, {});

if (JSON.stringify(pageFamilyCounts) !== JSON.stringify(expectedPageFamilyCounts)) {
  throw new Error(
...
async function loadFixtureHelpers(): Promise<Phase79FixtureHelpers> {
  ...
  try {
    imported = (await import(fixtureHelperModule)) as Partial<Phase79FixtureHelpers>;
  } catch (error) {
    throw new Error(
...
async function expectOneResponsiveTree(page: Page): Promise<void> {
  await expect(page.locator('[data-obpt-mobile-copy], [data-obpt-desktop-copy]')).toHaveCount(0);

  for (const selector of ['#cron-entry-detail', '#limiter-detail', '#audit-detail']) {
...
async function apply200PercentZoom(page: Page): Promise<void> {
  ...
  if (!original) throw new Error('200% zoom requires a configured Chromium viewport');
...
test.describe('Phase 79 connected page contracts', () => {
  test('overview preserves fixed current-to-continuity order and named destinations', async ({
    ...
    await expect(root.getByRole('heading', { level: 1, name: 'Overview' })).toHaveCount(1);
    await expect(root.getByRole('heading', { level: 2, name: 'Current attention' })).toBeVisible();
    ...
    await expect(root.getByRole('link', { name: 'Inspect in Oban Web' })).toBeVisible();
  ...
  test('cron first-open push, switch-close replace, Back, and direct reload preserve selection', async ({
    ...
    await page.getByRole('link', { name: fixtureState.cron.firstEntry, exact: true }).click();
    await expect(page).toHaveURL(
      exactUrl(`/ops/jobs/cron?entry=${encodeURIComponent(fixtureState.cron.firstEntry)}`)
    );
    await expect(page.locator('#cron-entry-detail')).toBeVisible();

    await page.getByRole('link', { name: fixtureState.cron.secondEntry, exact: true }).click();
  ...
  test('limiters use named table actions and canonical detail history without mutation affordances', async ({
    ...
    await expect(page.locator('#limiter-detail')).toBeVisible();
    ...
    await page.locator('#limiter-detail [data-obpt-detail-close]').click();
    await expect(page).toHaveURL(exactUrl('/ops/jobs/limiters'));
    await expect(page.locator('input[type="checkbox"]')).toHaveCount(0);
    await expect(page.getByRole('button', { name: /pause|resume|run|retry|execute/i })).toHaveCount(
      0
    );
  });
  ...
  test('audit filter-page-selection composition preserves scope and immutable evidence history', async ({
    ...
    const firstEvidence = page
      .locator(`#audit-record-${fixtureState.audit.firstEvent}`)
      .getByRole('link', { name: /View evidence/ });
    await expect(firstEvidence).toHaveAccessibleName(/View evidence for .+ on .+/);
    await firstEvidence.click();
    await expect(page).toHaveURL(new RegExp(`[?&]event=${fixtureState.audit.firstEvent}(?:&|$)`));
    await expect(page.locator('#audit-detail')).toBeVisible();

    const secondEvidence = page
      .locator(`#audit-record-${fixtureState.audit.secondEvent}`)
      .getByRole('link', { name: /View evidence/ });
    await expect(secondEvidence).toHaveAccessibleName(/View evidence for .+ on .+/);
    await secondEvidence.click();
    await expect(page).toHaveURL(new RegExp(`[?&]event=${fixtureState.audit.secondEvent}(?:&|$)`));
  ...
  test('cron one-dialog confirmation traps focus, handles Escape, and restores its action', async ({
    ...
    await expect(dialog.locator('[data-obpt-confirm-consequence]')).toBeFocused();
  ...
  test('cron recovery remains explicit and safe across expired, drifted, consumed, skipped, and partial results', async ({
    ...
    for (const recovery of [
      ...
      await expect(dialog).not.toContainText(/job ran|completed/i);
      await dialog.getByRole('button', { name: /Keep current schedule|Close/ }).click();
    }
  });
  ...
  test('cron duplicate activation produces one repository-backed receipt and Audit destination', async ({
    ...
    await page.getByRole('textbox', { name: 'Reason' }).fill('Planned maintenance');
    ...
    await expect(receipt.getByRole('link', { name: /Audit/ })).toHaveAttribute(
      'href',
  ...
  test('wide details remain nonmodal and resize as one persistent tree without a focus trap', async ({
    ...
    await detail.evaluate((element) => element.setAttribute('data-test-identity', 'phase79-detail'));
    ...
    await expect(detail).toHaveAttribute('data-test-identity', 'phase79-detail');
  ...
  test('authorization keeps read-only pages nonmutating and cron controls server-derived', async ({
    ...
    await expect(page.getByText(/not authorized|requires.*role/i)).toBeVisible();
    await expect(page.locator('#cron-confirmation-dialog')).toHaveCount(0);
  ...
  test('320 page reflow keeps named table actions, one tree, visible focus, and no overflow', async ({
    ...
    for (const path of [
      ...
      const root = page.locator('#overview-page, #cron-page, #limiters-page, #audit-page');
  ...
  test('reduced motion keeps detail and confirmation immediate without hiding content', async ({
    ...
    await expect(dialog.getByRole('button', { name: 'Keep running' })).toBeEnabled();
  });
