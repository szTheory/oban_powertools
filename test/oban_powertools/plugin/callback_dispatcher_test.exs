defmodule ObanPowertools.Plugin.CallbackDispatcherTest do
  ...
  setup do
    ...
    {:ok, pid} =
      start_supervised({CallbackDispatcher, conf: conf, name: Oban, interval: 60_000})
  ...
  describe "polling callbacks" do
    test "processes chain.step_succeeded callbacks successfully", %{dispatcher_pid: pid} do
      callback =
        insert_callback!("chain.step_succeeded", %{
    ...
    test "processes batch.completed callbacks successfully", %{dispatcher_pid: pid} do
      callback =
        insert_callback!("batch.completed", %{
          "batch_id" => Ecto.UUID.generate()
        })
    ...
    test "handles poison pill callbacks by marking them as failed and continuing", %{
      dispatcher_pid: pid
    } do
