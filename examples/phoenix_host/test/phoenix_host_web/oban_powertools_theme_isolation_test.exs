defmodule PhoenixHostWeb.ObanPowertoolsThemeIsolationTest do
  ...
  test "Powertools jobs page owns the scoped theme root and md5 assets", %{conn: conn} do
    ...
    assert has_element?(view, "[data-obpt-app-shell]")
    assert has_element?(view, ~s([data-obpt-nav-item="jobs"][data-obpt-current="true"]))
    assert has_element?(view, "#jobs-page")
    assert has_element?(view, "#jobs-filter[data-obpt-filter-bar]")
    assert has_element?(view, ~s(#jobs-state-available[aria-current="page"]))
    assert has_element?(view, ~s(#jobs-results[data-obpt-data-state="ready"]))
    assert has_element?(view, "#jobs-results-row-#{job.id}")
